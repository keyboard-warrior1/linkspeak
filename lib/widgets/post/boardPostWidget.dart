...
class _BoardPostWidgetState extends State<BoardPostWidget> {
  ...
  Widget buildTextField(String description, double deviceHeight) =>
      ConstrainedBox(
          constraints: BoxConstraints(
              minHeight: 10,
              maxHeight: widget.inPreview ? 400 : deviceHeight * 0.60,
              minWidth: 10,
              maxWidth: widget.inPreview ? 400 : deviceHeight * 0.45),
          child: Column(
  ...
  Widget buildMediaItem(String url, int mediaIndex, double deviceHeight,
    ...
    return Container(
        ...
        width: widget.inPreview ? 400 : deviceHeight * 0.45,
        child: ClipRRect(
  ...
  Widget build(BuildContext context) {
    ...
    final List<BoardPostItem> items = helper.boardPostItems;
    final backgroundColor = helper.boardPostBackground;
    final gradientColor = helper.boardPostGradient;
          ...
          child: ConstrainedBox(
            ...
            child: Column(
                ...
                children: <Widget>[
                  ...
                  if (!nsfwCondition && widget.inPreview || !widget.inPreview)
                    Expanded(
                        child: ListView.separated(
                            ...
                            shrinkWrap: true,
                            itemBuilder: (ctx, ind) {
                              var isText = items[ind].isText;
                              var description = items[ind].description;
                              var mediaURL = items[ind].mediaURL;
                              if (isText) {
                                return buildTextField(
                                    description, _deviceHeight);
                              } else {
                  ...
                  if (nsfwCondition)
                    const Divider(
                        color: Colors.transparent, indent: 0.0, endIndent: 0.0),
                  if (nsfwCondition)
                    TextButton(
...
class BoardPopupMenu extends StatefulWidget {
  final dynamic previewSetstate;
  final bool isInBoard;
  const BoardPopupMenu(this.previewSetstate, this.isInBoard);

  @override
  State<BoardPopupMenu> createState() => _BoardPopupMenuState();
}

class _BoardPopupMenuState extends State<BoardPopupMenu> {
  @override
  Widget build(BuildContext context) {
    final helper = Provider.of<FullHelper>(context);
    final helperNo = Provider.of<FullHelper>(context, listen: false);
    final bool isClubPost = helper.isClubPost;
    final postId = helper.postId;
    final isFav = helper.isFav;
    final title = helper.title;
    final postTopics = helper.postTopics;
    final postMedia = helper.postImgUrls;
    final postDate = helper.postedDate;
    final clubName = helper.clubName;
    final isMod = helper.isMod;
    final helperFav = helperNo.fav;
    final hidePost = helperNo.hidePost;
    final deletePost = helperNo.deletePost;
    final unhidePost = helperNo.unhidePost;
    return Container(
        padding: const EdgeInsets.all(2.5),
        decoration: BoxDecoration(
            color: widget.isInBoard ? Colors.black54 : Colors.transparent,
        ...
        child: !isClubPost
            ? PopupMenuButton(
                ...
                child: Icon(Icons.more_vert,
                    color: widget.isInBoard ? Colors.white : Colors.black),
                itemBuilder: (_) => [
                      PopupMenuItem(
                          padding: const EdgeInsets.all(0.0),
                          enabled: true,
...
class BoardPostBaseline extends StatefulWidget {
  ...
  @override
  State<BoardPostBaseline> createState() => _BoardPostBaselineState();
}

class _BoardPostBaselineState extends State<BoardPostBaseline> {
