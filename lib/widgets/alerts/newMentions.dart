...
class _NewMentionsState extends State<NewMentions> {
  ...
  String giveLastText() {
    if (widget.isPost) return 'post';
    if (widget.isComment || widget.isFlareComment) return 'comment';
    if (widget.isReply || widget.isFlareReply) return 'reply';
    if (widget.isBio || widget.isFlaresBio) return 'bio';
    if (widget.isFlare) return 'flare';
    return '';
  }
