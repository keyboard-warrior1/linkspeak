...
class _SendButtonState extends State<SendButton> with WidgetsBindingObserver {
  ...
  Future<void> pickAddressChatHandler(
      String myUsername, String shownName, dynamic point) async {
    ...
    batch.set(_myFriendChatDocument, {
      'displayMessage': 'shared a location',
      'isRead': false,
  ...
  void mediaButtonHandler(String _myUsername) {
        ...
        builder: (_) {
          ...
          final ListTile _camera = ListTile(
              ...
              onTap: () {
                if (assets.length < 10) {
                  Navigator.pop(context);
                  _chooseCamera(_primaryColor, _accentColor);
                }
              });
          ...
          final ListTile _customAddress = ListTile(
              horizontalTitleGap: 5.0,
              leading: const Icon(Icons.map, color: Colors.black),
              title: const Text('Send custom address',
                  style: TextStyle(color: Colors.black)),
              onTap: () {
                if (isBlocked && !_myUsername.startsWith('Linkspeak') ||
                    (imBlocked && !_myUsername.startsWith('Linkspeak'))) {
  ...
  Future<void> uploadMediaHandler(String _myUsername) async {
    if (!sendingMediaLoading) {
      setState(() {
      ...
      if (invalidSizeVid.isNotEmpty ||
        ...
        if (invalidVid) {
          _showDialog(Icons.warning, Colors.red, 'Invalid Video',
              "Shared videos can be up to 10 minutes long");
        }
      } else {
        final targetUser =
            await firestore.collection('Users').doc(widget.chatId).get();
  ...
  Future<void> sendAudioHandler(String _myUsername, dynamic _myFriend) async {
      ...
      await Future.delayed(
          ...
          () => batch.commit().then((value) {
                ...
                General.updateControl(
                    ...
                    docID: null,
                    docFields: {});
                EasyLoading.showSuccess('Audio sent',
                    dismissOnTap: true, duration: const Duration(seconds: 2));
                stopTimer = false;
                // if (widget.scrollController.hasClients) if (widget
                //         .scrollController.offset <=
                ...
                GestureDetector(
                    onTap: () async {
                      if (isBlocked && !_myUsername.startsWith('Linkspeak') ||
                        ...
                        uploadMediaHandler(_myUsername);
                      }
                    },
                    child: Padding(
                                  ...
                                  IconButton(
                                      ...
                                      onPressed: () {
                                        ...
                                        if (kIsWeb ||
                                          ...
                                          mediaButtonHandler(_myUsername);
                                        }
                                      }),
                                  IconButton(
                  ...
                  IconButton(
                      onPressed: () async {
                        if (isBlocked && !_myUsername.startsWith('Linkspeak') ||
                          ...
                          sendAudioHandler(_myUsername, _myFriend);
                        }
                      },
