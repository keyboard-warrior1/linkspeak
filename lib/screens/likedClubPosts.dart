...
class LikedClubPosts extends StatefulWidget {
  const LikedClubPosts();

  @override
  _LikedClubPostsState createState() => _LikedClubPostsState();
}

class _LikedClubPostsState extends State<LikedClubPosts>
  ...
  void initState() {
    ...
    final setLikedPosts =
        Provider.of<MyProfile>(context, listen: false).setLikedClubPosts;
    _getLikedPosts = getLikedPosts(myUsername, setLikedPosts);
    _scrollController =
        Provider.of<LikeScreenScrollProvider>(context, listen: false)
            .likedClubScrollController;
    _disposeScrollController =
        Provider.of<LikeScreenScrollProvider>(context, listen: false)
            .disposeLikedClubController;
    _scrollController.addListener(() async {
      if (_scrollController.position.pixels ==
        ...
        if (isLastPage) {
          ...
          if (!isLoading) {
            getMorePosts(myUsername, setLikedPosts);
          }
        }
      }
    });
  ...
  Widget build(BuildContext context) {
    ...
    final setLikedPosts =
        Provider.of<MyProfile>(context, listen: false).setLikedClubPosts;
    final clearLikedPosts =
        Provider.of<MyProfile>(context, listen: false).clearLikedClubPosts;
    final myUsername =
        Provider.of<MyProfile>(context, listen: false).getUsername;
    final bool selectedAnchorMode =
        Provider.of<ThemeModel>(context, listen: false).anchorMode;
    const Widget emptyBox = const SizedBox(
      width: 0,
      height: 0,
    );
    super.build(context);
                ...
                SizedBox(
                  ...
                  child: TextButton(
                    ...
                    onPressed: () {
                      ...
                      setState(() {
                        _getLikedPosts =
                            getLikedPosts(myUsername, setLikedPosts);
                      });
                    },
                    child: Center(
                      child: Text(
                      ...
                      RefreshIndicator(
                        ...
                        child: Noglow(
                          child: ListView.separated(
                            shrinkWrap: true,
                            key: PageStorageKey<String>('LikedClubsPosts'),
