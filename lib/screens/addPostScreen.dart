    ...
    Future<void> _sendPost() async {
      ...
      if (invalidSizeVid.isNotEmpty || invalidSizeIMG.isNotEmpty) {
        ...
        if (invalidSizeVid.isNotEmpty) {
          _showDialog(Icons.info_outline, Colors.blue, lang.clubs_newPost3,
              lang.clubs_newPost4);
        }
        if (invalidSizeIMG.isNotEmpty) {
          _showDialog(Icons.info_outline, Colors.blue, lang.clubs_newPost5,
              lang.clubs_newPost6);
        }
      } else {
        ...
        if (last24hrs.length >= 50) {
          ...
          Future<void> mentionHandler(String mentionedUser) async {
            ...
            if (mentionedUser != username) {
              ...
              final String friendMessage = General.giveMentionPost(targetLang);
              final userExists = targetUser.exists;
              if (userExists) {
                final notifDescription = '$username $friendMessage';
          ...
          Future<void> uploadPlace(String placeName) async {
            ...
            batch.set(
                targetPostDoc,
                {'date': rightNow, 'point': location, 'clubName': ''},
                SetOptions(merge: true));
            batch.set(
                targetPlaceDoc,
          ...
          return batch.commit().then((_) async {
            ...
            if (hasNSFW) {
              reviewalDoc.set({
                ...
                EasyLoading.showSuccess(lang.clubs_newPost10,
  ...
  Widget build(BuildContext context) {
    ...
    String? _validateDescription(String? value) {
      if ((value!.isEmpty ||
              value.replaceAll(' ', '') == '' ||
              value.trim() == '') &&
          assets.isEmpty) return lang.clubs_newPost1;
      if (value.length > 10000) return lang.clubs_newPost2;
      if ((value.isEmpty ||
              value.replaceAll(' ', '') == '' ||
              value.trim() == '') &&
    ...
    return Form(
        ...
        child: SizedBox(
            child: Padding(
                padding:
                    EdgeInsets.only(top: _deviceHeight * 0.05, bottom: 65.0),
                child: Column(
                                      ...
                                      children: <Widget>[
                                        ...
                                        if (!kIsWeb)
                                          AdditionalAddressButton(
                                            ...
                                            changeAddress: changePostAddress,
                                            changeAddressName:
                                                changePostAddressName,
                                            postLocation: postAddress,
                                            postLocationName: postAddressName,
                                          ),
                                        ...
                                        Container(
                                            ...
                                            child: ElevatedButton(
                                                ...
                                                child: Center(child: isLoading ? SizedBox(height: 15, width: 15, child: CircularProgressIndicator(color: _accentColor, strokeWidth: 1.50)) : Text(lang.clubs_newPost18, style: TextStyle(color: _accentColor, fontSize: 30.0)))))
                                      ])
                                ])),
                            const BoardTab(),
                            const BranchTab()
...
class NewPostMedia extends StatelessWidget {
  final dynamic scrollController;
...
class NewPostTopics extends StatelessWidget {
                  ...
                  onTap: () {
                    ...
                    if (isLoading) {
                      ...
                      showModalBottomSheet(
                          ...
                          builder: (_) {
                            return AddTopic(
                                _addTopic, _formTopics, false, true, false);
                          });
                    }
                  },
                  child: TopicChip(lang.clubs_newPost22,
          ...
          if (!myTopicsAdded)
            Container(
                child: GestureDetector(
                    ...
                    child: TopicChip(lang.clubs_newPost24,
                        Icon(Icons.add, color: _accentColor), () {
                      if (isLoading) {
                      } else {
                        addMyTopics(_myTopics);
...
class _BoardTabState extends State<BoardTab>
  ...
  Future<void> _addPost(
      ...
      required List<String> formTopics,
      required bool commentsDisabled,
      required void Function(String) profileAddPost}) async {
    final lang = General.language(context);
      ...
      if (last24hrs.length >= 50) {
        ...
        if (invalidVid) {
          ...
          return batch.commit().then((_) async {
            ...
            EasyLoading.showError(lang.clubs_newPost11,
  ...
  Widget buildListTile(bool isNsfwTile, double deviceWidth) {
    final lang = General.language(context);
    return Container(
        width: deviceWidth,
        color: Colors.white,
                          ...
                          Container(
                              ...
                              child: Noglow(
                                  child: ListView(
                                    ...
                                    if (allowed)
                                      buildAdder(lang.clubs_newPost25,
                                          Icons.note_outlined, () {
                                    ...
                                    if (allowed && !kIsWeb)
                                      buildAdder(lang.clubs_newPost26,
                                    ...
                                    buildAdder(lang.clubs_newPost22, Icons.add,
                                        () {
                                      if (isLoading) {
                                      } else {
                                        showModalBottomSheet(
                                    ...
                                    if (!myTopicsAdded)
                                      buildAdder(lang.clubs_newPost24,
                                          Icons.add, _addMyTopics),
                                  ]))),
                              ...
                              child: ElevatedButton(
                                  ...
                                  child: Center(
                                      child: Padding(
                                          ...
                                          child: isLoading ? SizedBox(height: 35, width: 35, child: const CircularProgressIndicator(color: Colors.white, strokeWidth: 1.50)) : Text(lang.clubs_newPost18, style: const TextStyle(color: Colors.white, fontSize: 30.0)))))),
                        ])))));
  }
      ...
      Container(
          ...
          child: Noglow(
              child: ListView(
                  ...
                  children: <Widget>[
                if (allowed)
                  buildAdder(lang.clubs_newPost27, Icons.note_outlined, () {
                    void handler(String desc) {
                      final noteItem = BoardPostItem(
                          isText: true,
                          mediaIsAsset: false,
                ...
                if (allowed && !kIsWeb)
                  buildAdder(lang.clubs_newPost26, Icons.image_outlined, () {
                    FocusScope.of(context).unfocus();
                    if (isLoading) {
                    } else {
                      showModalBottomSheet(
                          ...
                          builder: (_) {
                            final ListTile _choosephotoGallery = ListTile(
                                ...
                                onTap: () => _choose(
                                    systemPrimary, systemSecondary, lang));
                            final ListTile _camera = ListTile(
                                horizontalTitleGap: 5.0,
                                leading: const Icon(Icons.camera_alt,
                                    color: Colors.black),
                ...
                if (!myTopicsAdded)
                  buildAdder(lang.clubs_newPost24, Icons.add, _addMyTopics),
              ]))),
