...
class AddFlareComment extends StatefulWidget {
  final String poster;
  ...
  @override
  State<AddFlareComment> createState() => _AddFlareCommentState();
}

class _AddFlareCommentState extends State<AddFlareComment> {
  ...
  Future<void> _choose(String myUsername, dynamic lang) async {
    ...
    if (_result != null) {
      ...
      _myImgUrl =
          'Flare Comments/${widget.poster}/${widget.collectionID}/${widget.flareID}/$myUsername/$iD';
      containsMedia = true;
      if (mounted) {
        setState(() {});
      }
  ...
  Future<void> addComment(
    ...
    final checkExists = await General.checkExists(
        'Flares/$posterUsername/collections/${widget.collectionID}/flares/${widget.flareID}');
    if (checkExists) {
      setState(() {
        isLoading = true;
      });
      ...
      final currentPostComments = thisFlare.collection('comments');
      final myUser = firestore.collection('Users').doc(_username);
      final myUserComments = myUser.collection('Flare Comments');
      ...
      Future<void> mentionHandler(String mentionedUser) async {
        ...
        if (mentionedUser != _username) {
          ...
          final String friendMessage = General.giveMentionComment(targetLang);
        ...
        if (lastHourComments.length >= 30) {
          ...
          if (containsMedia) {
            ...
            if (fileSize > 15000000) {
              ...
              if (filter.hasProfanity(originalDescription)) {
                batch.update(firestore.doc('Profanity/Flare Comments'),
                    {'numOfProfanity': FieldValue.increment(1)});
                batch.set(
                    firestore
                        .collection('Profanity/Flare Comments/Comments')
                        .doc(),
                    {
              ...
              await storage
                ...
                batch.set(myUser, {'flare comments': FieldValue.increment(1)},
                    SetOptions(merge: true));
                batch.set(
                    firestore.collection('Flares').doc(posterUsername),
                    {'numOfComments': FieldValue.increment(1)},
                    SetOptions(merge: true));
                batch.update(
                    thisCollection, {'comments': FieldValue.increment(1)});
                batch.update(thisFlare, {'comments': FieldValue.increment(1)});
                ...
                return batch.commit().then((value) async {
                  ...
                  final status = targetUser.get('Status');
                  if (status != 'Banned') {
                    if (targetUser.data()!.containsKey('AllowFlareComments')) {
                      final allowComments =
                          targetUser.get('AllowFlareComments');
                      if (allowComments) {
                        if (posterUsername != _username) {
                          ...
                          secondBatch.set(
                              firestore
                                  .collection('Flares')
                                  .doc(posterUsername),
                              {'commentNotifs': FieldValue.increment(1)},
                              SetOptions(merge: true));
                        }
                      }
                    } else {
            ...
            batch.set(myUser, {'flare comments': FieldValue.increment(1)},
                SetOptions(merge: true));
            batch.update(thisCollection, {'comments': FieldValue.increment(1)});
            batch.update(thisFlare, {'comments': FieldValue.increment(1)});
            Map<String, dynamic> fields = {
              'flare comments': FieldValue.increment(1)
            };
            ...
            return batch.commit().then((value) async {
              ...
              if (status != 'Banned') {
                if (targetUser.data()!.containsKey('AllowFlareComments')) {
                  final allowComments = targetUser.get('AllowFlareComments');
                  if (allowComments) {
                    if (posterUsername != _username) {
                      secondBatch.set(otherCommentsNotifs.doc(), {
                        'poster': widget.poster,
  ...
  Widget build(BuildContext context) {
    ...
    String? _validateComment(String? value) {
      if ((value!.isEmpty ||
              ...
              value.trim() == '') &&
          !containsMedia) {
        return lang.flares_addComment1;
      }
      if (value.length > 1500) {
        return lang.flares_addComment2;
      }
      return null;
    }
        ...
        child: TypeAheadFormField<MiniProfile>(
            ...
            textFieldConfiguration: TextFieldConfiguration(
                ...
                decoration: InputDecoration(
                    hintText: lang.flares_addComment7,
                    counterText: '',
    ...
    final Widget _addComment =
      ...
      TextButton(
          ...
          child: (isLoading)
              ...
              : Center(
                  child: Text(lang.flares_addComment8,
                      style: TextStyle(
                    ...
                    children: <Widget>[
                        Center(
                            child: Container(
                                ...
                                child: Text(lang.flares_addComment9,
                                    softWrap: true,
                                    style: const TextStyle(
